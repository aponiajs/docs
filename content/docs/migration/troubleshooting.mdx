...
Every handler argument must carry a parameter decorator. A handler with no
decorators receives the whole request context as its only argument; a handler
that mixes decorated and undecorated parameters receives `undefined` for the
undecorated ones. A named selection such as `@Query("term")` is also
`undefined` when the request omits it.
